  ...
  <div class="picture-item" @mouseleave="isShow = false">
    <el-card :body-style="{ padding: '0px' }">
      <div class="img-wrap">
        ...
        <div class="shade" v-show="picture.genre == 'video'">
          <i class="el-icon-video-play" @click="playVideo(picture)"></i>
        </div>
      </div>
      <div style="padding: 14px;">
        <p>{{ picture.title[lang] }}</p>
  ...
  methods: {
    ...
    delPicture () {
      this.$confirm('此操作将永久删除该图片信息, 是否继续?', '提示', {
        ...
        delKindResources({ uuid: this.picture.uuid }).then(res => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    playVideo (picture) {
      this.$emit('playVideo', picture.video_url)
    },
    edit () {
      console.log(this.picture)
      updateKindResources(this.picture).then(res => {
        if (res.success) {
          this.$emit('loadData')
        }
      })
