<template>
  <div class="item" @mouseleave="isShow = false" @mouseenter="isShow = true">
    ...
    <div class="handler" v-show="isShow">
      <span v-if="activeFeild == 'sound'" @click="userSound">使用声音</span>
      <span v-else @click="userImg">使用图片</span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    userSound () {
      ...
      this.$emit('userSound', this.item.sound)
    },
    // 使用图片
    userImg () {
      console.log(this.item.image)
      this.$emit('userImg', this.item.image)
    },
    playVoice () {
      let audio = new Audio()
      if (!this.isPlay) {
